...
mod tests {
    ...
    fn test_sort_entries_uses_tuple_order() {
        let mut entries: Vec<Entry> = vec![
            ...
            (3, PathBuf::from("c.txt"), false),
            (1, PathBuf::from("a.txt"), false),
            (5, PathBuf::from("e.txt"), false),
            (2, PathBuf::from("d.txt"), false),
            (7, PathBuf::from("g.txt"), false),
            (8, PathBuf::from("h.txt"), false),
        ];
        ...
        assert_eq!(
            ...
            [
                ...
                (2, PathBuf::from("d.txt")),
                (3, PathBuf::from("c.txt")),
                (5, PathBuf::from("e.txt")),
                (6, PathBuf::from("f.txt")),
                (7, PathBuf::from("g.txt")),
                (8, PathBuf::from("h.txt")),
            ]
        );
    }
    ...
    fn test_sort_entries_tuple_order_orders_bool() {
        ...
        let ordered: Vec<_> = entries
            .into_iter()
            .map(|(modified, path, is_dir)| (modified, path, is_dir))
            .collect();
        assert_eq!(
            ordered,
            [
